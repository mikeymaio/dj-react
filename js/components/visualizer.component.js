...
const Visualizer = React.createClass({
  ...
  */
  _onRenderStyleDefault () {
    ...
    const maxBarNum = Math.floor((cy * 2 * Math.PI) / (barWidth + barSpacing));
    const slicedPercent = Math.floor((maxBarNum * 25) / 100);
    const barNum = maxBarNum - slicedPercent;
    const freqJump = Math.floor(frequencyData.length / (maxBarNum / 25));
    ...
    for (let i = 0; i < barNum; i++) {
      ...
      const x = 0;
      const y = cy - (amplitude / 2 ); // - barHeight
      const w = barWidth;
      const h = amplitude;
